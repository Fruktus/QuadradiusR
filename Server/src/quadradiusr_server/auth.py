...
class Auth:
    ...
    async def login(self, username: str, password: bytes) -> Optional[User]:
        ...
        if check_password(password, user.password_):
            return user
        else:
            return None
    ...
    def authenticate(self, token: str) -> Optional[str]:
        try:
            ...
            return decoded['usr']['id']
        except jwt.InvalidTokenError:
            return None

    async def get_user(self, user_id: str) -> Optional[User]:
        db_user = await self.repository.user_repository.get_by_id(user_id)
        if not db_user:
            return None
        return User(db_user)
...
def check_password(password: bytes, hash_: str) -> bool:
